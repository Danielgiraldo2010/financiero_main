// features/integracion/exportaciones/presupuesto/ui/ExportarPresupuestoDialog.tsx

import { useState } from "react"
import { useExportarPresupuesto } from "../hook"
...
export function ExportarPresupuestoDialog({ open, onClose }: Props) {
  const [vigencia, setVigencia] = useState<string>(String(new Date().getFullYear()))
  const { exportar, isPending, isPolling, estado, reset } = useExportarPresupuesto()
        ...
        {!estado && (
          <div className="space-y-3">
            <div className="space-y-1">
              <Label htmlFor="pres-vigencia">Vigencia</Label>
              <Input
                id="pres-vigencia"
                type="number"
                value={vigencia}
        ...
        {(isPending || isPolling) && !finalizado && (
          <div className="flex flex-col items-center gap-3 py-4">
            ...
            <p className="text-sm text-muted-foreground">
              {isPending ? "Iniciando exportación..." : "Procesando exportación de presupuesto..."}
            </p>
          </div>
        )}
