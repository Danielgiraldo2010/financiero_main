// features/integracion/exportaciones/chip/ui/ExportarChipDialog.tsx

import { useState } from "react"
import { useExportarCHIP } from "../hook"
...
export function ExportarChipDialog({ open, onClose }: Props) {
  const [vigencia, setVigencia] = useState<string>(String(new Date().getFullYear()))
  const { exportar, isPending, isPolling, estado, reset } = useExportarCHIP()
  ...
  return (
    <Dialog open={open} onOpenChange={(o) => !o && handleClose()}>
      <DialogContent className="max-w-sm">
        <DialogHeader>
          <DialogTitle>Exportar al CHIP</DialogTitle>
        </DialogHeader>

        {!estado && (
          <div className="space-y-3">
            <div className="space-y-1">
              <Label htmlFor="chip-vigencia">Vigencia</Label>
              <Input
                id="chip-vigencia"
                type="number"
                value={vigencia}
        ...
        {(isPending || isPolling) && !finalizado && (
          <div className="flex flex-col items-center gap-3 py-4">
            ...
            <p className="text-sm text-muted-foreground">
              {isPending ? "Iniciando exportación..." : "Procesando exportación CHIP..."}
            </p>
          </div>
        )}
