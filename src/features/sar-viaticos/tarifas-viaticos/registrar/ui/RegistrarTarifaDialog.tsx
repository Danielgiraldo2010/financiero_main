...
import {
  RegistrarTarifaViaticoSchema,
  type RegistrarTarifaViaticoFormValues,
} from '../../../model/schema';
import {
...
import { useRegistrarTarifaViatico } from '../hook';
...
export function RegistrarTarifaDialog({ open, onClose }: Props) {
  const mutation = useRegistrarTarifaViatico();
  const form = useForm<RegistrarTarifaViaticoFormValues>({
    ...
    defaultValues: {
      ...
      zona:                'LOCAL',
      municipioTipo:       'CAPITAL',
      incluyePernoctacion: false,
      horasMinimasDict:    null,
      valorDiaCompleto:    0,
      valorMedioDia:       0,
      valorTransporte:     0,
      normaAplicable:      '',
  ...
  return (
    <Dialog open={open} onClose={onClose} title="Registrar Tarifa de Viatico" maxWidth="lg">
      <form onSubmit={onSubmit} className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          {/* FIX: zona ahora es SelectField con valores exactos del backend */}
          ...
          />
          {/* FIX: municipioTipo ahora es SelectField con valores exactos del backend */}
          ...
          render={({ field, fieldState }) => (
            <FormField label="Norma aplicable" error={fieldState.error?.message}>
              <input className="w-full border rounded px-3 py-2 text-sm"
                placeholder="Ej: Decreto 1323/2019" {...field} />
            </FormField>
          )}
        />
        ...
        {mutation.isError && (
          <p className="text-sm text-destructive">Error al registrar tarifa.</p>
        )}
        <div className="flex justify-end gap-2 pt-2">
          <Button type="button" variant="outline" onClick={onClose}>Cancelar</Button>
          <Button type="submit" disabled={mutation.isPending}>
            {mutation.isPending ? 'Guardando...' : 'Registrar tarifa'}
          </Button>
        </div>
      </form>
    </Dialog>
